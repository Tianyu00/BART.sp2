...
mc.wbart <- function(
    ...
    sigma2_prior_a = 10.0, sigma2_prior_b = 1.0,
    tau2_prior_a = 2.0, tau2_prior_b = 1.0,
    range = 4.6, smoothness = 0.0,
    tau2 = 2.0,
    range_prior_mean = 5.0, range_prior_sd = 1.5,
    smoothness_prior_mean = 0.0, smoothness_prior_sd = 0.5,
    coordinates='ll'
    )
{
    ...
    for(i in 1:mc.cores) {
        parallel::mcparallel({tools::psnice(value=nice);
                   wbart(x.train=x.train, y.train=y.train, x.test=x.test,
                         ...
                         range_select_sd = range_select_sd,
                         smoothness_select_sd = smoothness_select_sd,
                         ...
                         tau2 = tau2,
                         range_prior_mean = range_prior_mean,
                         range_prior_sd = range_prior_sd,
                         smoothness_prior_mean = smoothness_prior_mean,
    ...
    if(mc.cores==1 | attr(post, 'class')!='wbart') return(post)
        ...
        for(i in 2:mc.cores) {
            # 3 new parameters estimated: tau2, smoothness, range
            post$smoothness <- cbind(post$smoothness,
                                     post.list[[i]]$smoothness)
            post$range <- cbind(post$range,
                                post.list[[i]]$range)
            post$tau2 <- cbind(post$tau2,
                               post.list[[i]]$tau2)
            post$w <- cbind(post$w,
                            post.list[[i]]$w)
            post$w_test <- cbind(post$w_test,
                                 post.list[[i]]$w_test)
